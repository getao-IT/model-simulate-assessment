...
public class SimulateTaskController {
    ...
    @ApiImplicitParam(name = "taskId", paramType = "query", value = "仿真任务id", required = true)
    public CommonResult<JSONObject> getModelAssessmentInfo(int taskId) {
        JSONObject result = simulateTaskService.getModelAssessmentInfo(taskId);
        return new CommonResult<JSONObject>().success().data(result).message("获取模型评估结果信息成功");
    }
}
