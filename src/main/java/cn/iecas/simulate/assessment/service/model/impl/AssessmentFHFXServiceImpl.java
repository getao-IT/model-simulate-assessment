 ...
 * @description 府会分析模型仿真数据分析评估服务实现类
 */
...
public class AssessmentFHFXServiceImpl implements AssessmentService<SimulateDataInfo> {
    ...
    */
    ...
    private double buildSingleFourIndexAssessment(IndexInfo indexInfo, List<SimulateDataInfo> assessmentDatas, JSONArray contents) {
        double score = 0;
        if (indexInfo.getIndexName().equalsIgnoreCase("政要丑闻类型")) {
        ...
        if (indexInfo.getIndexName().equalsIgnoreCase("政要丑闻数量")) {
            JSONObject type = new JSONObject();
        ...
        if (indexInfo.getIndexName().equalsIgnoreCase("议员政策法案类型")) {
            JSONObject type = new JSONObject();
            Set<String> territorys = assessmentDatas.stream().map(SimulateDataInfo::getTerritory).collect(Collectors.toSet());
        ...
        if (indexInfo.getIndexName().equalsIgnoreCase("议员政策法案数量")) {
            JSONObject type = new JSONObject();
